...
export function letrasparametroscuadrados(valor) {
    const unidades = [
        '', 'uno', 'dos', 'tres', 'cuatro', 'cinco', 'seis', 'siete', 'ocho', 'nueve',
        'diez', 'once', 'doce', 'trece', 'catorce', 'quince', 'dieciséis', 'diecisiete', 'dieciocho', 'diecinueve'
    ];
    ...
    function convertircentenas(num) {
        if (num === 100) return 'cien';
        let centenasvalor = Math.floor(num / 100);
        let decenasvalor = num % 100;
        let resultado = '';
        if (centenasvalor > 0) {
            resultado = centenas[centenasvalor];
        }
        if (decenasvalor > 0) {
            if (decenasvalor < 20) {
                resultado += (resultado ? ' ' : '') + unidades[decenasvalor];
            } else {
                let dec = Math.floor(decenasvalor / 10);
                let uni = decenasvalor % 10;
                if (dec === 2 && uni > 0) {
                    resultado += (resultado ? ' ' : '') + 'veinti' + unidades[uni];
                } else {
                    resultado += (resultado ? ' ' : '') + decenas[dec];
                    if (uni > 0) resultado += ' y ' + unidades[uni];
                }
    ...
    const entero = parseInt(partes[0], 10);
    const decimal = partes[1];
    const textoentero = convertirmiles(entero).trim() || 'cero';
